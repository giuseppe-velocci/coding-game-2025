...
{
    public class OrderApiHandler : AsbtractHttpApiClientService, ICrudHandler<Order, WriteOrder>
    {
        private readonly string _endpointUrl;
        private readonly static JsonSerializerOptions _serializerOptions = new() { PropertyNameCaseInsensitive = true };

        public OrderApiHandler(IHttpClientFactory httpClientFactory, ApiGateConfig config, ILogger<OrderApiHandler> logger) :
            base(httpClientFactory, logger)
        {
            _endpointUrl = $"{config.OrderApiEndpoint}/orders";
        }

        public async Task<OperationResult<long>> Create(WriteOrder value, CancellationToken cts)
        {
            try
            {
        ...
        public async Task<OperationResult<Order[]>> ReadAll(CancellationToken cts)
        {
            ...
            var responseContent = await response.Content.ReadAsStringAsync(cts);
            var statusCode = (int)response.StatusCode;
            return GetOperationResultResponse<Order[]>(statusCode, responseContent)!;
        }
        ...
        public async Task<OperationResult<None>> Update(long id, WriteOrder value, CancellationToken cts)
        {
            try
            {
                var httpClient = _httpClientFactory.CreateClient();
                var response = await _retryPolicy.ExecuteAsync(() =>
                    httpClient.PutAsJsonAsync($"{_endpointUrl}/{id}", value, cts));
