...
{
    public class ProductApiHandler : AsbtractHttpApiClientService, ICrudHandler<Product, WriteProduct>
    {
        private readonly string _endpointUrl;
        private readonly static JsonSerializerOptions _serializerOptions = new() { PropertyNameCaseInsensitive = true };

        public ProductApiHandler(IHttpClientFactory httpClientFactory, ApiGateConfig config, ILogger<ProductApiHandler> logger) :
            base(httpClientFactory, logger)
        {
            _endpointUrl = $"{config.ProductApiEndpoint}/products";
        }

        public async Task<OperationResult<long>> Create(WriteProduct value, CancellationToken cts)
        {
            try
            {
        ...
        public async Task<OperationResult<Product[]>> ReadAll(CancellationToken cts)
        {
            ...
            var responseContent = await response.Content.ReadAsStringAsync(cts);
            var statusCode = (int)response.StatusCode;
            return GetOperationResultResponse<Product[]>(statusCode, responseContent)!;
        }
        ...
        public async Task<OperationResult<None>> Update(long id, WriteProduct value, CancellationToken cts)
        {
            try
            {
