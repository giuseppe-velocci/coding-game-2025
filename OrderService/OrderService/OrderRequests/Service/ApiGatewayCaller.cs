...
namespace OrderService.OrderRequests.Service
{
    public class ApiGatewayCaller(IHttpClientService _httpClient, ApiEndpointConfig _config) : IApiGatewayCaller
    {
        ...
        {
            var value = await _httpClient.CallEndpointAsync<SuccessResult<Address>>($"{_config.ApiGatewayEndpoint}/addresses/{id}", cts);
            return value?.Value;
        }

        public async Task<Product?> GetProductAsync(long id, CancellationToken cts)
        {
            var value = await _httpClient.CallEndpointAsync<SuccessResult<Product>>($"{_config.ApiGatewayEndpoint}/products/{id}", cts);
            return value?.Value;
        }

        public async Task<User?> GetUserAsync(long id, CancellationToken cts)
        {
            var value = await _httpClient.CallEndpointAsync<SuccessResult<User>>($"{_config.ApiGatewayEndpoint}/users/{id}", cts);
            return value?.Value;
        }
    }
}
